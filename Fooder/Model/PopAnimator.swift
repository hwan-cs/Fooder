...
//        let vc = UIStoryboard.init(name: "Main", bundle: Bundle.main).instantiateViewController(withIdentifier: "CardViewController")
        guard let fromVC = transitionContext.viewController(forKey: .from) else { return }
        guard let mainVC = fromVC as? CardViewController else { return }
...
//            fromVC.view.frame.origin.y = UIScreen.main.bounds.height
    ...
    func animationForDismiss(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        guard let mainVC = toVC as? CardViewController else { return }
        guard let selectedCell = mainVC.selectedCell else { return }
...
//            toVC.view.frame.origin.y = UIScreen.main.bounds.height - toVC.view.frame.height
        }) { completed in
            transitionContext.completeTransition(completed)
        }
    }
